resource "azurerm_network_interface" "nics" {
  ...
  enable_accelerated_networking                                 = var.req_arth_nic_config[count.index].enable_accelerated_networking
  tags                                                          = var.req_arth_vm_config.tag

  ip_configuration {
    name                                                        = var.req_arth_nic_config[count.index].config_name
    ...
    private_ip_address_allocation                               = var.req_arth_nic_config[count.index].private_ip_address_allocation
  }
}
...
resource "azurerm_virtual_machine" "virtual_machine" {
  ...
  os_profile_linux_config {
    disable_password_authentication                             = var.req_arth_vm_config.disable_password_authentication
  }
...
resource "azurerm_virtual_machine_data_disk_attachment" "data_disk_attach" {
  ...
  lun                                                           = count.index
  caching                                                       = var.req_arth_data_disk[count.index].caching#var.req_arth_vm_config.caching
